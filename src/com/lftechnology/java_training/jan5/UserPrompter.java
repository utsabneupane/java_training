...
public class UserPrompter {
	private static final Logger LOGGER = Logger.getLogger(UserPrompter.class.getName());
	...
	/**
	 * it takes values from user and checks to specified value and prompt until user enter specified that value
	 */
	private static void valueCheck() {
		try {
			...
			} while (!input.next().equalsIgnoreCase(EXIT_KEY));
		} finally {
			input.close();
		}
	}
